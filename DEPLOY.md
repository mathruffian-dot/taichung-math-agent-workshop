...
原本 reveal.js、wordcloud2、qrcode、supabase-js 都走 jsdelivr CDN。
**學校網路擋 CDN 的話，reveal.js 載不到就整份簡報打不開。**

現在全部下載到 `vendor/`（約 200KB），零 CDN 依賴：

| 檔案 | 用途 |
|---|---|
| `vendor/reveal.js` `reveal.css` `reset.css` `night.css` | 簡報引擎 |
| `vendor/wordcloud2.min.js` | 文字雲繪製 |
| `vendor/qrcode.min.js` | QR 生成 |
| `vendor/supabase.umd.js` | 互動後端（UMD 單檔版，自包含） |

> 只剩 Google Fonts 還走外網，但那只影響字體，載不到會 fallback 到系統中文字型，功能不受影響。

**附帶好處**：整個資料夾複製到 USB，離線也能開簡報（只有互動需要網路）。

### 4K 大屏的縮放上限

Reveal 預設 `maxScale: 2`，在 3840×2160 的觸控大屏上只會放大 2 倍，
畫面縮在中間、四周留黑邊。已改為 **3.2**（4K 需要 3.09）。

各解析度的實際縮放：

| 投影解析度 | 縮放 |
|---|---|
| 1024×768（4:3 舊投影機） | 1.07 |
| 1280×800 | 1.10 |
| 1920×1080 | 1.54 |
| 3840×2160（4K 大屏） | 3.09 |

### 使用者輸入一律轉義

文字雲的投稿內容原本直接串進 innerHTML。學員輸入含 `<` 的內容
（例如「數學 < 國文」）會顯示錯誤，惡意輸入可執行腳本。現在一律經過 escape。

### 視窗小於 435px 會變捲動模式

Reveal 5 的 `scrollActivationWidth` 預設 435px。全螢幕投影不會遇到，
但**講者若把瀏覽器視窗縮很小，簡報會變成捲動排版**。這是正常行為，放大視窗即可恢復。
